...
The [Integration Layer documentation][docs] is the model: what an extension is, the
...
The docs say `parent` is "the representation the Integration Layer resolved, plus
...
an external system by it. The encoding is internal to the Integration Layer and is
free to change.
...
non-null in the Integration Layer's schema, so both are safe; check before keying by a
...
`serve` and `invoke` have no Integration Layer to read configuration from, so they take
...
for each recommendation — still comes from the Integration Layer.
...
sequenceDiagram
    ...
    Note over R: The recommended products are stubs.<br/>Their other fields are owned by the<br/>Integration Layer, so plan an entities fetch.
    ...
    Note over R: Merge the Algolia ranking with the<br/>Integration Layer's resolved fields.
    R-->>SF: product + fully-resolved recommendations
```

The trust boundary never moves. The `_entities` re-entry runs through the same
session-scoped loader as a direct read, and the Integration Layer re-checks the URL's
...
```
examples/<name>  ──push──▶  Integration Layer              extension runtime
