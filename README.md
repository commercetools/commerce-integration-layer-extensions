...
commercetools Integration Layer.

This repo **complements** the [Integration Layer documentation][docs] — it doesn't
...
> The Integration Layer is in public beta and its documentation is not yet on the
...
# Get the templates
git clone https://github.com/commercetools/integration-layer-extensions
cd integration-layer-extensions && pnpm install
...
| --- | --- | --- |
| **server-time** | schema | A brand-new type and root field (`Query.serverTime`) that shares nothing with the Integration Layer |
| **loyalty-points** | schema | A field on an entity computed from an *argument* (`Product.loyaltyPoints`) — no Project data read |
| **price-discount** | schema | A field computed from a nested value the Integration Layer owns, via `@requires` (`Product.discountedPrice`) |
...
| `EXTENSION_CONFIG_<KEY>` | Local-only: feeds `ctx.config.<KEY>` to `serve` and `invoke`, which have no Integration Layer to read config from ([details](docs/authoring.md#local-development)) |
