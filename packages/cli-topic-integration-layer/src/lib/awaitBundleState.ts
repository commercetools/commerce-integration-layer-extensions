...
//
//   - It never treats "I couldn't find out" as a failure. A Commerce Integration Layer that
...
export async function awaitBundleState(
  ...
  for (;;) {
    ...
    if (meta.state === undefined) {
      // A Commerce Integration Layer that doesn't report state. Nothing to wait for.
      return {
        kind: "unknown",
        reason: "this Commerce Integration Layer does not report bundle status",
        meta,
      };
    }
