...
// Commerce Integration Layer — so a query like `{ product(id:…) { name loyaltyPoints(price:…) }}`
// resolves `name` from the Commerce Integration Layer and `loyaltyPoints` from the local
...
// source carries an anonymous session bearer (the Commerce Integration Layer re-validates the
 ...
 * every integration-layer subgraph request; the Commerce Integration Layer 403s a bearer
