...
// (packages/tooling/src/loadBundle.ts). The bundle is esbuild's single-file
// CommonJS artifact, so we evaluate it with the CommonJS calling convention and a
// `require` that resolves only `graphql` (a second copy would break `instanceof`).
// It runs as an ordinary module — not a security boundary; the runtime enforces
// that, and `staticAnalysis.ts` screens the source first.
