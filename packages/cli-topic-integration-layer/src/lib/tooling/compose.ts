...
//
// The LOW-LEVEL `composeWithIntegrationLayer` is copied verbatim from
// commercetools/integration-layer-extension-examples (packages/tooling/src/compose.ts).
// The env/token-minting `fetchIntegrationLayerSubgraphSdl` wrapper is NOT copied —
// fetching the project's subgraph SDL is a plain authenticated HTTP call, so it lives
// in `../ilClient.ts` parameterised by (baseUrl, projectKey, token), and the command
// layer supplies the token.
