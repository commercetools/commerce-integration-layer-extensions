...
describe("mintSession", () => {
  // The integration layer mounts the session router at `/:projectKey/session`
  ...
  it("posts to /<project>/session — NOT /<project>/token", async () => {
    ...
    const session = await mintSession(AUTH, "octolog-b2b", { kind: "anonymous" });

    expect(session.token).toBe("anon-token");
    expect(session.describe).toBe("anonymous");
    expect(calls[0].url).toBe(`${AUTH}/octolog-b2b/session`);
  ...
  it("logs a customer in with an ORDINARY password grant — the storefront's own flow", async () => {
    ...
    const session = await mintSession(AUTH, "octolog-b2b", {
      kind: "password",
  ...
  it("surfaces the integration layer's own error description, not a bare status", async () => {
...
describe("mintSession presentment", () => {
  ...
  it("lets one field be chosen without forcing the other two", async () => {
    // Each is independently defaulted by the integration layer, so `--currency EUR`
