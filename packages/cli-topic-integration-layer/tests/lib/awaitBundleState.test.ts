...
// Commerce Integration Layer, an extension that isn't deployed, a metadata read that errored
...
describe("awaitBundleState", () => {
  ...
  it("treats a Commerce Integration Layer that reports no state as unknown, not failed", async () => {
    const c = clock();
