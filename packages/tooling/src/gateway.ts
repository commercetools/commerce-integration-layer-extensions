// The `--gateway` half of `ee-ext serve`: an executable merged schema that routes
// each field to its owner — the local extension and the project's deployed
...
//
// We use Apollo Gateway (the reference Federation implementation) because the
// extension model leans on `@interfaceObject` and only the reference gateway resolves
// that correctly. Both subgraphs are reached over HTTP, so the extension runs as a
// genuine subgraph; the integration-layer data source carries an anonymous session
// bearer (the integration layer re-validates the bearer's project against the URL).
...
export async function mintAnonymousSession(
  ...
  const url = `${base}/api/${encodeURIComponent(projectKey)}/token`;
