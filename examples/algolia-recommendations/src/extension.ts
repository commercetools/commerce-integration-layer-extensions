 ...
 * id); the router re-enters the Commerce Integration Layer by that key (the `_entities` query)
 * for every other product field. So Algolia decides *which* products to recommend; the
 * Commerce Integration Layer remains the source of truth for product *detail*.
 *
 ...
 *
 * WHY NOT OVERRIDE SEARCH. The obvious cousin of this example — `@override`ing the
 * discovery field to rank search with Algolia — has no faithful form.
 * `Query.search: ProductSearchConnection!` is built from SHARED value types
 * (`PageInfo`, `ProductEdge`) that every other connection reuses, so overriding them
 * seizes them graph-wide. See the shared-result-type trap in ../../../docs/authoring.md.
 * `examples/category-counts-override` shows `@override` on a field where it does work.
 *
 * HTTP via a STANDARD SDK. Uses the official `algoliasearch` client directly. The
 * runtime exposes a global `fetch` limited to a per-project host allowlist, so allow
 * Algolia's hosts first:
 *
 *     commercetools integration-layer allowlist add '*.algolia.net' '*.algolianet.com'
 *
 * The SDK's fetch transport then uses it transparently. The Algolia
 ...
 * `commercetools auth login`). See ../../../docs/authoring.md.
 */
...
export const typeDefs = `
  ...
  type ProductRecommendation {
    "The recommended product itself, as a federation entity — the resolver returns only its \`id\` (the raw CT product id from Algolia's objectID) and the router re-enters the Commerce Integration Layer, which resolves it (leniently accepting a raw id) and fills in the rich catalog data (the join target)."
    product: Product!
...
export const resolvers = {
  Product: {
    // `product` is the entity representation the Commerce Integration Layer resolved — its
    ...
    // back from the Commerce Integration Layer (the join target).
    recommendations: async (
      ...
      try {
        ...
        return hits
          ...
          // ourselves — the encoding is internal to the Commerce Integration Layer.
