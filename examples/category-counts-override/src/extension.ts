 ...
 *
 * The Commerce Integration Layer already serves `Query.categoryProductCounts` (how many
 ...
 *     }
 *
 * (`@shareable` does NOT work here: the Commerce Integration Layer does not mark its own copy
 ...
 * types graph-wide and breaks every other field that uses them — see the
 * shared-result-type trap in ../../../docs/authoring.md.
 *
 * ── What you take on ───────────────────────────────────────────────────────────
...
export const typeDefs = `
  ...
  type Query {
    "Product counts per category subtree, served from this project's own search index instead of the Commerce Integration Layer's. Same signature as the field it replaces."
  ...
  "An entity, so only its key is declared here — the resolver returns { id } stubs and the router fetches the rest from the Commerce Integration Layer."
