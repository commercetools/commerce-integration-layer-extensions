 ...
 * non-null in the Commerce Integration Layer's schema (as is `Category.key`), so that is safe
 * here — check before keying by an optional `key`.
 *
 * The same one-line shape works for the other entities in the entity catalog —
 * `Customer`, `Category`, `Order`, `Cart`, `Quote`, `ShoppingList`, … — this template
 * just happens to pick the B2B one.
 *
 * ── Runtime shape ──────────────────────────────────────────────────────────────
