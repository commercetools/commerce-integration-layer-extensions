...
`extension pull` downloads the project's served bundle (`GET /extension/bundle`) to a file — defaulting to the stored filename under `./dist`, overridable with `--out` — and reports the version and source revision it fetched. Previously only the bundle's metadata was reachable (via `extension status`), never the code.
